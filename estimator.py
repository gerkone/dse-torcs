...
class Estimator:
    def __init__(self, dataset_dir, testset_dir, load_old, resnet):
    ...
    def run(self):
        for i, ep_file in zip(range(len(self.dataset_files) - 1), self.dataset_files[12:]):
    ...
    def evaluate(self):
        for i, ep_file in zip(range(len(self.evaluation_files) - 1), self.evaluation_files):
            print("loading episode data: {} - {}/{}".format(ep_file, i, len(self.evaluation_files)))
            episode = np.load(ep_file, allow_pickle = True)
...
if __name__ == "__main__":
    ...
    est = Estimator(args.dataset_dir, args.testset_dir, args.load_old, args.resnet)
